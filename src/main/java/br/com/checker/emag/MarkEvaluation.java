...
import br.com.checker.emag.SpecificRecommendation.MarkRecommendation;
...
public class MarkEvaluation extends Evaluation {
	...
	private List<Occurrence> checkRecommendation2() {
		List<Occurrence> occurrences = new ArrayList<Occurrence>();
		occurrences.add(new Occurrence("2", false,this.getDocument().getFirstElement().toString()));
	...
	public List<Occurrence> checkRecommendation9() {
		...
		if(!hasBlankLink)
			occurrences.add(new Occurrence("9", false,this.getDocument().getFirstElement().toString()));
